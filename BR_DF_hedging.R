...
nadwyzkiEUR<-subset(ForEUR,Zostalo>10000)
...
Forwardy7<-transform(DplyRApproach3,Rozl77=MaFunkcja_zob(DataRozl))
Forwardy77<-group_by(Forwardy7,Rozl77)%>% summarise(Amount=sum(suma))
Forwardy77<-subset(Forwardy77,Rozl77>today()-7 & Rozl77<today()+120)
Forwardy77_nal<-subset(Forwardy77_nal,Rozl77>today()-7 & Rozl77<today()+120)
...
wykres<-ggplot() + 
  ...
  geom_text(data=melted,aes(x=Data2[10],y=round(NAL_sredni_wplyw/1000+200),label = "ŚREDNI TYGODNIOWY WPŁYW EUR"),size=3)+
  #geom_label(data=melted,aes(x=Data2,y=value,label=value),alpha=0.1)+
  #geom_text(aes(y = mid_y))
  geom_segment(data=melted3,aes(x=Data3,y=0,xend=Data3,yend=value),stat="identity",size=3,color="sienna",alpha=0.2)+
  scale_x_date(labels = date_format("%Y-%m-%d"),date_breaks ="1 week",name="Data rozliczenia" )+
  scale_y_continuous(name="Wartość w EUR (tys.)",limits=(c(0,max(max(tabela2$Liabilities/1000+500),max((tabela2$Receivables+tabela2$Amount.x)/1000+500)))),
                                                         breaks =c(500,1000,1500,2000,2500,3000,3500,4000,4500,5000,5500,6000,6500,7000,7500,8000,8500,9000))+
  theme_light()+theme(legend.position="bottom")
 

colnames(tabela2)[6]<-"Pokr_forw"
colnames(tabela2)[7]<-"Pokr_nalezn"
