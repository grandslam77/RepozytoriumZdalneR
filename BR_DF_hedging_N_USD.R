...
naleznosci5<-subset(naleznosci5,Wal.=="USD")
...
zobowiazania2<-subset(zobowiazania2,Wal.=="USD")
...
ForUSD$PokrNależn[is.na(ForUSD$PokrNależn)] <- ""
ForUSD_zakup<-subset(ForUSD,PokrNależn=="")
ForUSD_nat<-subset(ForUSD,PokrNależn!="")

#DplyRApproach2<-group_by(ForEUR,DataRozl,PokrNależn)%>% summarise(suma=sum(WartośćWal), śRednia=mean(KursForw))
DplyRApproach3<-group_by(ForUSD_zakup,DataRozl)%>% summarise(suma=sum(WartośćWal))

DplyRApproach3<-group_by(ForUSD_zakup,DataRozl)%>% summarise(suma=sum(WartośćWal))
DplyRApproach3a<-group_by(ForUSD_zakup,DataRozl)%>% summarise(suma=sum(WartośćWal),sumaPLN=sum(WrtWalKr))

DplyRApproach4<-group_by(ForUSD_nat,DataRozl)%>% summarise(suma=sum(WartośćWal))
DplyRApproach4a<-group_by(ForUSD_nat,DataRozl)%>% summarise(suma=sum(WartośćWal),sumaPLN=sum(WrtWalKr))
...
#zobowiazania
to_plot_for<-data.frame(Data=tabela2$Data,ZobowiazanieUSD=round(tabela2$Liabilities/1000,0))
...
wykres<-ggplot() + 
  ...
  scale_y_continuous(name="Wartość w USD (tys.)",limits=(c(0,max(max(tabela2$Liabilities/1000+500),max((tabela2$Receivables+tabela2$Amount.x)/1000+500)))),
                     breaks =c(100,150,200,250,300,350,400,450,500,500,600,600,700,700,800,850,900))+
